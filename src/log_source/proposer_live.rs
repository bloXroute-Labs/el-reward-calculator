...
const INPUT_JSON_PATH: &str = "data/proposer_inputs.json";
...
#[derive(Debug, Deserialize, Clone)]
struct UltrasoundAdjustmentEnvelope { data: Vec<UltrasoundAdjustment> }
...
pub fn run_proposer_compare_from_json_and_write<T: RewardStats>(
    ...
    // read inputs and map slot -> best input (must have proposer_send_timestamp_ms)
    let entries_by_slot = read_input_entries(INPUT_JSON_PATH.into())?;
    let mut slot_to_entry: HashMap<u64, InputEntry> = HashMap::new();
    for slot in &slots {
        if let Some(mut vv) = entries_by_slot.get(slot).cloned() {
            // keep only entries with proposer_send_timestamp_ms
            vv.retain(|e| e.proposer_send_timestamp_ms.as_deref().map(|s| !s.trim().is_empty()).unwrap_or(false));
            if vv.is_empty() {
                warn!("proposer_live: slot {} has no proposer_send_timestamp_ms in {}", slot, INPUT_JSON_PATH);
                continue;
            }
            // pick earliest timestamp if many
            vv.sort_by(|a, b| a.proposer_send_timestamp_ms.cmp(&b.proposer_send_timestamp_ms));
            slot_to_entry.insert(*slot, vv[0].clone());
        } else {
            warn!("proposer_live: no input found for slot {} in {}", slot, INPUT_JSON_PATH);
        }
    }
    if slot_to_entry.is_empty() {
        warn!("proposer_live: nothing to analyze (no slots had proposer_send_timestamp_ms)");
        return Ok(());
    }

    // build a Tokio runtime (since the rest of your pipeline is sync)
    let rt = tokio::runtime::Runtime::new()?;
    let (analyses, relays_by_ts) = rt.block_on(async {
        ...
        let mut relays_by_ts_all: HashMap<u64, HashMap<String, HashMap<String, Value>>> = HashMap::new();
    ...
    let json_path = format!("{}/proposer_compare_{}_{}.json", folder_path.trim_end_matches('/'), date_str, time_str);
    let jf = File::create(&json_path)?;
    serde_json::to_writer_pretty(jf, &analyses)?;
    eprintln!("wrote {}", json_path);

    // write comparison CSV vs calculator outputs
    let csv_path = format!("{}/proposer_compare_{}_{}.csv", folder_path.trim_end_matches('/'), date_str, time_str);
    ...
    for (slot_str, calc) in per_slot_selected {
        let slot_u64 = match slot_str.parse::<u64>() { Ok(s) => s, Err(_) => continue };
        let prop = match analyses.get(&slot_u64) { Some(p) => p, None => continue };
        ...
        let prop_host = prop.onchain_bid_delivered_relay.clone().unwrap_or_default();
...
async fn analyze_one_slot_exact_ts(
    ...
    let slot = entry.slot;
    let ts = entry.proposer_send_timestamp_ms.clone()
        .ok_or_else(|| anyhow!("slot {} missing proposer_send_timestamp_ms", slot))?;
    ...
    for (ep, res) in fetches {
        ...
        match res {
            Ok(mut rows) => {
                ...
                for (r, raw) in rows {
                    all_rows_with_src.push(RowWithSrc { row: r, raw, src: label.clone() });
                }
            }
            Err(e) => {
                endpoints_used.push(label.clone());
    ...
    let onchain_value_eth = entry.header_block_value
    ...
    let ts_matched_with_src: Vec<RowWithSrc> = all_rows_with_src
        ...
        .filter(|r| {
            ...
            let Some(h) = onchain_hash.as_deref() else { return true; };
    ...
    let matched_by_timestamp = {
        ...
        for r in &ts_matched_with_src { uniq.insert(r.src.clone()); }
        uniq.len()
    };
    ...
    for r in &ts_matched_with_src { inner.insert(r.src.clone(), r.raw.clone()); }
    relays_by_ts.insert(ts.clone(), inner);

    // delivery flags
    let delivered_any = block_hash_in_rows(onchain_hash.as_deref(), &ts_matched_with_src); // tightened to ts-matched set for strictness
                        ...
                        if let Some(adj_val_s) = adj.adjusted_value.as_deref() {
                            ...
                            if let Some(obj) = r.raw.as_object_mut() {
                                ...
                                if let Some(sub) = adj.submitted_value.as_deref() {
                                    obj.insert("submitted_value".to_string(), Value::String(sub.to_string()));
                                }
                            }
                        }
                    }
    ...
    let equal_to_proxy_bidders = if is_equal_to_proxy_bid {
        equal_relays_ts.iter().cloned().collect::<Vec<_>>().join(",")
    } else { String::new() };
    ...
    if let Some(ref ocw) = onchain_value_wei {
        for (v, src, _bh) in &competitor_values {
            if v < ocw {
                let better = match &second_highest { Some((cur, _)) => v > cur, None => true };
                if better {
    ...
    let is_winning_bid_highest = if let (Some(ref ocw), Some((ref maxv, _))) = (&onchain_value_wei, &max_competitor) {
        ocw >= maxv
    } else { true };

    // details (from ts-matched rows first, then any exact-hash row in ts-matched)
    let mut block_number: Option<String> = ref_rows.iter().find_map(|r| r.row.block_number.clone());
    let mut proposer_pubkey: Option<String> = ref_rows.iter().find_map(|r| r.row.proposer_pubkey.clone());
    let mut parent_hash: Option<String> = ref_rows.iter().find_map(|r| r.row.parent_hash.clone());
    let mut builder_pubkey: Option<String> = ref_rows.iter().find_map(|r| r.row.builder_pubkey.clone());
    let mut proposer_fee_recipient: Option<String> = ref_rows.iter().find_map(|r| r.row.proposer_fee_recipient.clone());
    ...
    if block_number.is_none() {
        if let Some(rws) = find_row_by_block_hash(onchain_hash.as_deref(), &ref_rows) {
            ...
            proposer_fee_recipient = rws.row.proposer_fee_recipient.clone().or(proposer_fee_recipient);
        }
    }
    ...
    let mut final_is_proxy_win = entry.win.as_deref().map(|w| w.eq_ignore_ascii_case("yes")).unwrap_or(true);
    if forced_proxy_loss_due_equal { final_is_proxy_win = false; }

    let (el_inc_wei_opt, el_inc_eth_opt, el_inc_pct_int_opt, el_inc_pct_prec_opt, el_improve_eth) =
        if forced_proxy_loss_due_equal {
            (Some("0".to_string()), Some("0".to_string()), Some(0),
             Some("0.000000000000000000000000000".to_string()), Some("0".to_string()))
            ...
            if let (Some(ref ocw), Some((ref sh, _))) = (&onchain_value_wei, &second_highest) {
                ...
                (Some(wei_s), Some(inc_eth.clone()), Some(pct_int), Some(pct_prec), Some(inc_eth))
            } else {
                let zero = "0".to_string();
                (Some(zero.clone()), Some(zero.clone()), Some(0),
                 Some("0.000000000000000000000000000".to_string()), Some(zero))
            }
        } else {
            // legacy: improvement if not delivered by any endpoint; here we’ve already restricted set to exact TS
            let improvement_eth = if !delivered_any {
                if let (Some(maxw), Some(inw)) = (
                    ref_rows.iter().filter_map(|r| r.row.value.as_deref()).filter_map(parse_bigint).max_by(bigint_cmp),
                    onchain_value_wei.clone()
                ) {
                    let diff = &maxw - &inw;
                    if diff.is_negative() || diff.is_zero() { Some("0".to_string()) } else { Some(wei_to_eth_str(&diff)) }
                } else { None }
            } else { Some("0".to_string()) };
        ...
        if final_is_proxy_win {
            ...
            if uplift_pct > dec!(0) && uplift_eth > dec!(0) {
                if uplift_pct <= dec!(1) {
                    ...
                    if uplift_eth >= dec!(0.0015) { fee = dec!(0.0015); }
                } else if uplift_pct <= dec!(9) {
                    if uplift_eth > dec!(0.003) { fee = dec!(0.003); }
                    else if uplift_eth > dec!(0.0015) { fee = dec!(0.0015); }
                } else {
                    if uplift_eth > dec!(0.005) { fee = dec!(0.005); }
                    else if uplift_eth > dec!(0.003) { fee = dec!(0.003); }
                    else if uplift_eth > dec!(0.0015) { fee = dec!(0.0015); }
                }
            }
        }
    ...
    let rproxy_input_win_marked_lost_due_equal =
        entry.win.as_deref().map(|w| w.eq_ignore_ascii_case("yes")).unwrap_or(true) && forced_proxy_loss_due_equal;
...
async fn fetch_rows(client: &reqwest::Client, ep: &Endpoint, slot: u64) -> Result<Vec<(RelayRow, Value)>> {
    let url_with_slot = if ep.url.contains('?') { format!("{}&slot={slot}", ep.url) } else { format!("{}?slot={slot}", ep.url) };
    let mut req = client.get(&url_with_slot);
    if let Some(k) = &ep.api_key { req = req.header("X-API-Key", k); }
    let mut resp = req.send().await.with_context(|| format!("GET {}", url_with_slot))?;

    if !resp.status().is_success() {
        let status = resp.status();
        if matches!(status, StatusCode::BAD_REQUEST | StatusCode::NOT_FOUND | StatusCode::UNAUTHORIZED) {
            let mut req2 = client.get(&ep.url);
            if let Some(k) = &ep.api_key { req2 = req2.header("X-API-Key", k); }
            sleep(Duration::from_millis(100)).await;
            resp = req2.send().await.with_context(|| format!("GET {}", ep.url))?;
        }
    }
...
async fn fetch_ultrasound_adjustments(
    ...
    let url = format!("https://relay-analytics.ultrasound.money/ultrasound/v1/data/adjustments?slot={}", slot);
    let resp = client.get(&url).send().await.with_context(|| format!("GET {}", url))?;
    if !resp.status().is_success() { return Err(anyhow!("HTTP {} from {}", resp.status(), url)); }
    let text = resp.text().await?;
    let v: Value = serde_json::from_str(&text).with_context(|| "parse ultrasound adjustments JSON")?;

    if let Ok(env) = serde_json::from_value::<UltrasoundAdjustmentEnvelope>(v.clone()) { return Ok(env.data); }
    if let Ok(single) = serde_json::from_value::<Vec<UltrasoundAdjustment>>(v.clone()) { return Ok(single); }
...
fn row_slot_eq(r: &RelayRow, slot: u64) -> bool {
    match r.slot.as_deref() { Some(s) => s == slot.to_string(), None => false }
}

fn block_hash_in_rows(input_hash: Option<&str>, rows: &[RowWithSrc]) -> bool {
    let Some(needle) = input_hash else { return false; };
    let needle = needle.trim().to_lowercase();
    rows.iter().any(|rs| {
        rs.row.block_hash.as_deref()
            .map(|h| h.trim().eq_ignore_ascii_case(&needle))
            .unwrap_or(false)
    })
}

fn find_row_by_block_hash(input_hash: Option<&str>, rows: &[RowWithSrc]) -> Option<RowWithSrc> {
    let Some(needle) = input_hash else { return None; };
    let needle = needle.trim().to_lowercase();
    rows.iter().find(|rs| {
        rs.row.block_hash.as_deref().map(|h| h.trim().eq_ignore_ascii_case(&needle)).unwrap_or(false)
    }).cloned()
}

fn is_proxy_node_id_str(s: &str) -> bool { s.to_ascii_lowercase().contains("proxy") }

fn has_proxy_node_id(row: &RowWithSrc) -> bool {
    if let Some(id) = row.raw.get("node_id").and_then(|v| v.as_str()) { return is_proxy_node_id_str(id); }
    if let Some(ref id) = row.row.node_id { return is_proxy_node_id_str(id); }
    false
}
...
fn read_input_entries(path: PathBuf) -> Result<Vec<InputEntry>> {
    // detect array by first non-whitespace char
    ...
    if first_non_ws == Some(b'[') {
        ...
        let entries: Vec<InputEntry> = serde_json::from_reader(f).with_context(|| "parse JSON array")?;
        return Ok(entries);
    }
    ...
    for (i, line) in buf.lines().enumerate() {
        ...
        let v: InputEntry = serde_json::from_str(&l).with_context(|| format!("parse NDJSON line {}", i + 1))?;
        out.push(v);
    }
    Ok(out)
}

fn read_input_entries_map(path: PathBuf) -> Result<HashMap<u64, Vec<InputEntry>>> {
...
fn resolve_endpoints_from_env() -> Vec<Endpoint> {
    ...
    let endpoints: Vec<String> = if let Ok(s) = std::env::var("RELAY_ENDPOINTS") {
        ...
        vec![
            "https://relay-analytics.ultrasound.money/relay/v1/data/bidtraces/proposer_header_delivered".to_string(),
        ]
    };
    ...
    let mut keys: Vec<String> = if let Ok(s) = std::env::var("RELAY_API_KEYS") {
        ...
        vec![
            "fcd8ed67ecf8d0ec3bece1da340c899c84104e71abcbeee702013fbfebe95416".to_string(),
        ]
    };

    if keys.is_empty() {
        return endpoints.into_iter().map(|u| Endpoint { url: u, api_key: None }).collect();
    } else if keys.len() == 1 {
        let k = keys.remove(0);
        return endpoints.into_iter().map(|u| Endpoint { url: u, api_key: Some(k.clone()) }).collect();
    } else if keys.len() >= endpoints.len() {
        return endpoints.into_iter().zip(keys.into_iter()).map(|(u, k)| Endpoint { url: u, api_key: Some(k) }).collect();
    } else {
        // fewer keys than endpoints -> reuse first key
        let k0 = keys.remove(0);
        return endpoints.into_iter().map(|u| Endpoint { url: u, api_key: Some(k0.clone()) }).collect();
    }
}
...
fn local_stamp() -> (String, String) {
    let now = Local::now();
    (now.format("%Y-%m-%d").to_string(), now.format("%Hh_%Mm_%Ss").to_string())
}

fn parse_bigint(s: &str) -> Option<BigInt> { s.parse::<BigInt>().ok() }
fn bigint_cmp(a: &BigInt, b: &BigInt) -> Ordering { a.cmp(b) }
...
fn eth_str_to_wei(eth: &str) -> BigInt {
    ...
    let (int_part, frac_part) = match parts.len() { 1 => (parts[0], ""), _ => (parts[0], parts[1]) };
    let mut frac = frac_part.to_string();
    if frac.len() > 18 { frac.truncate(18); } else { frac.push_str(&"0".repeat(18 - frac.len())); }
    let mut full = String::new();
    full.push_str(if int_part.is_empty() { "0" } else { int_part });
    full.push_str(&frac);
    let full = full.trim_start_matches('0');
    let wei = if full.is_empty() { BigInt::zero() } else { full.parse::<BigInt>().unwrap_or_else(|_| BigInt::zero()) };
    if neg { -wei } else { wei }
}
...
fn percent_precise(numer: &BigInt, denom: &BigInt, scale: usize) -> (String, i64) {
    ...
    if s.starts_with('-') {
        let mut body = s[1..].to_string();
        if body.len() <= scale {
            let zeros = "0".repeat(scale - body.len() + 1);
            body = format!("{}{}", zeros, body);
        }
        let split = body.len() - scale;
        let out = format!("-{}.{:}", &body[..split], &body[split..]);
        // integer floor not needed by caller, but we keep parity with your earlier fn
        return (out, 0);
    }
    if s.len() <= scale {
    ...
    (out, s[..split].parse::<i64>().unwrap_or(0))
}
...
fn read_input_entries(path: PathBuf) -> Result<Vec<InputEntry>> {
    ...
    if first_non_ws == Some(b'[') {
        let f = File::open(&path)?;
        let entries: Vec<InputEntry> = serde_json::from_reader(f).with_context(|| "parse input JSON array")?;
        return Ok(entries);
    }
