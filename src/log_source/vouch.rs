...
/// Converts U256 wei to Decimal ETH
   pub fn u256_wei_to_eth_decimal(wei: U256) -> Decimal {
       let wei_str = wei.to_string(); // Convert U256 to base-10 string
       let wei_decimal = Decimal::from_str(&wei_str).unwrap_or(Decimal::ZERO);
       let eth = wei_decimal / dec!(1_000_000_000_000_000_000); // divide by 1e18
       eth
   }
...
fn process_json_first_pass(log_entry: &LogEntryVouch, slot_infos: &mut SlotInfos) {
    ...
    bid.bid_value = u256_wei_to_eth_decimal(wei);
...
pub fn calculate_uplift_second_pass_and_store_removed(
    ...
    for (_slot, slot_info_map) in slot_infos.iter_mut() {
        for (_slot_uid, slot_info) in slot_info_map.iter_mut() {
            if slot_info.is_proxy_win && slot_info.onchain_bid_value > Decimal::ZERO {
                ...
                if let Some(highest) = highest_bid {
                    ...
                    slot_info.second_highest_bid_value =
                        second_best_bid.map_or(Decimal::ZERO, |b| b.bid_value);
                    slot_info.second_higher_bid_delivered_relay =
                        second_best_bid.map_or_else(String::new, parse_url);

                    if slot_info.second_highest_bid_value > Decimal::ZERO {
