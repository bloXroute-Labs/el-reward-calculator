...
fn process_json(log_entry: &LogEntry, slot_infos: &mut SlotInfos){
    ...
    match log_entry.message.method.as_str() {
        "getHeader" => {
            match log_entry.message.msg.as_str() {
                "bid received" => {
                ...
                "received payload from relay" => {
                    ...
                    if let Some(slot_info_map) = slot_infos.get_mut(&slot) {
                        if let Some(slot_info) = slot_info_map.get_mut(&slot_uid) {
                            ...
                            println!("Bid length: {}", slot_info.info.bids.len());
                            ...
                            for bid in &slot_info.info.bids {
                                ...
                                if bid.block_hash == log_entry.message.blockHash {
                                    // Find all relays that provided the highest bid
                                    ...
                                    // Check if relay-proxy is among the highest bidders
                                    ...
                                    let highest_bidder_urls: Vec<_> = highest_bidders
                                        .iter()
                                        .map(|relay| {
                                            match Url::parse(relay) {
                                                Ok(url) => url.host_str().unwrap_or_default().to_string(),
                                    ...
                                    // join the parsed host strings of all highest bidders
                                    let highest_bidder_urls: Vec<_> = highest_bidders
                                        .iter()
                                        .map(|relay| {
                                            match Url::parse(relay) {
                                    ...
                                    slot_info.equal_to_proxy_bidders = relay_proxy_bidders.join(", ");
                                    slot_info.is_equal_to_proxy_bid = !relay_proxy_bidders.is_empty();
                                    // Set proxy win status
                                    slot_info.is_proxy_win = relay_proxy_won && !slot_info.is_equal_to_proxy_bid;

                                    let highest_bid = slot_info.info.bids.get(0).unwrap();
                                        slot_info.is_winning_bid_highest = if log_entry.message.blockHash.clone() == highest_bid.block_hash.clone() {
                                            true
                                        } else {
                                            match slot_info.info.bids.iter().find(|bid| bid.block_hash == log_entry.message.blockHash) {
                                                Some(winning_bid) => winning_bid.bid_value == highest_bid.bid_value,
                                                None => false,
                                            }
                                        };

                                    // Skip extra processing if multiple highest bids exist and relay-proxy didn't win
                                    if highest_bidders.len() > 1 && !relay_proxy_won {
                                        println!("Skipping processing as relay-proxy didn't win and multiple highest bids exist.");
                                        return;
                                    }
                                    ...
                                    if slot_info.is_proxy_win {
                                        ...
                                        slot_info.second_higher_bid_delivered_relay = second_best_bid
                                            .map_or(String::new(), |bid| {
                                                match Url::parse(&bid.relay) {
                                                    Ok(url) => url.host_str().unwrap_or_default().to_string(),
                                                    Err(_) => String::new(), // In case of invalid URL, return an empty string
                                                }
                                            });

                                        // Calculate EL reward improvement
                                        if !slot_info.is_equal_to_proxy_bid && slot_info.second_highest_bid_value > Decimal::ZERO {
                                            ...
                                            // Calculate fee per block based on EL reward preceise percentage
...
pub fn cleanup_slots_without_proxy(slot_infos: &mut SlotInfos) {
    ...
    // To avoid mutability issues during iteration, collect to remove later
    let mut slots_to_remove: Vec<(String, String)> = Vec::new();
